...
async def add_to_cart(
    ...
    return await cart_crud.add_to_cart(db, user_id, data)
...
async def remove_item(
    ...
    user_id = int(user["sub"])  # ✅ Convert sub (user.id) to int
    return await cart_crud.remove_item(db, user_id, product_id)
...
async def clear_cart(
    ...
    user_id = int(user["sub"])  # ✅ Convert sub (user.id) to int
    return await cart_crud.clear_cart(db, user_id)